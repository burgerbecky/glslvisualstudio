...
/***************************************
	...
	By using a simple state machine, parse the
	text as if it's a C++ file and remove all empty
	lines, whitespace and comments.

***************************************/

void BURGER_API StripComments(OutputMemoryStream *pOutput,const char *pInput,WordPtr uLength)
{
	enum eState {
		REMOVING_WHITESPACE,
		AFTER_TOKEN,
	...
	if (uLength) {
		...
		do {
			...
			case ALLOW_TEXT:
				...
				if (uCharacter=='\r') {
					if (uLength && reinterpret_cast<const Word8 *>(pInput)[0] == '\n') {
						++pInput;
						--uLength;
					}
					uCharacter = '\n';
			...
			case SLASH_COMMENT:
				...
				if (uCharacter=='\\') {
					...
					// for completeness and it really should issue an error condition.
					// Thankfully, visual studio issues a warning for source containing
					// this case
					if (uLength) {
					...
					// If it got here, it was a false alarm, discard the character
				}
...
/***************************************
	...
	Simple utility to convert a shader source file into a header
	that can be included for direct compilation into an
	application.
...
int BURGER_ANSIAPI main(int argc,const char **argv)
{
	...
	if (argc>=0) {
		...
		WordPtr uLength;
		const char *pInputBuffer = static_cast<const char*>(FileManager::LoadFile(&InputFilename,&uLength));
		...
		if (!pInputBuffer) {
			...
			if (CPP.GetValue()) {
				pInputBuffer = static_cast<const char *>(Output.Flatten(&uLength));
				Output.Clear();
				...
				// Strip the extension
				char *pExtension = StringCharacterReverse(BaseName.GetPtr(),'.');
				if (pExtension) {
					pExtension[0] = 0;
				}
				...
				if (!uLength) {
					...
					do {
						...
						--uLength;
						if (uTemp=='\r') {
							if (uLength && reinterpret_cast<const Word8 *>(pWork)[0]=='\n') {
								++pWork;
								--uLength;
							}
							uTemp='\n';
